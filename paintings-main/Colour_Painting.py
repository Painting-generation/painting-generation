...
class Painting:
    ...
    def __init__(self, img_path, strokes_count=100):
    ...
    def init_strokes(self, strokes_count):
        # Modify to use the provided strokes_count if available
        if strokes_count is not None:
            self.strokes_count = strokes_count
        for index in range(0, self.strokes_count):
    ...
    def mutate(self):
        ...
        if mutateOption == 0:
            index = self.find_stroke_index(mutatedStroke)
            # remove swapped brushStroke form strokes
            copyStrokes.pop(index)
            ...
            # fix all indices
            for i in range(len(copyStrokes)):
    ...
    def find_stroke_index(self, target_stroke):
        for i, stroke in enumerate(self.strokes):
            if (
                stroke.color == target_stroke.color
                and stroke.size == target_stroke.size
                and stroke.posY == target_stroke.posY
                and stroke.posX == target_stroke.posX
                and stroke.rotation == target_stroke.rotation
                and stroke.brush_type == target_stroke.brush_type
            ):
                return i
        return -1
    ...
    def mse(self, imageA, imageB):
        # The MSE function of two images calculated by the formula:
        # MSE = 1/N * sum((I1 - I2)^2)
        # where I1 and I2 are the two images and N is the total number of pixels
    ...
    def drawAll(self, strokes):
        ...
        for i in range(len(strokes)):
            if isinstance(strokes[i], Brush_stroke):
...
class Brush_stroke:
    ...
    def new_brush_type(self, maxBrushNumber):
        new_brush_type = random.randrange(0, maxBrushNumber)
